...
`https://paxai.app/mcp/agents/<name>` with the JWT as the bearer
token.
...
curl -sS -X POST https://paxai.app/api/v1/keys \
...
```bash
curl -sS -X POST https://paxai.app/auth/exchange \
...
await manager.connectToServer('ax', {
  url: new URL('https://paxai.app/mcp/agents/YOUR_AGENT_NAME'),
  requestInit: {
    headers: {
